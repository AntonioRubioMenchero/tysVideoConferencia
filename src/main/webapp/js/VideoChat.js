class VideoChat {
	constructor(ko) {
		...
		this.ws.onmessage = function(event) {
			...
			if (data.type=="ANSWER") {
				...
				self.addMensaje("Llamada aceptada", "purple");
				return;
			}
			if (data.type=="DENY") {
				self.addMensaje("La llamada ha sido rechazada", "purple");
				alert("Se ha rechazado la llamada");
	...
	rechazarLlamada(remitente, sessionDescription) {
		let self=this;
		this.addMensaje("Llamada de " + remitente + " rechazada", "purple");
		let msg = {
				type : "DENY",
				remitente : remitente
			};
